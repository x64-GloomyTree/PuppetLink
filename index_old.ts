...
const settings = definePluginSettings({
    flickerEnabled: {
        description: "Enable the letter flicker effect",
        type: OptionType.BOOLEAN,
        default: true
    },
    flickerColor: {
        description: "The color letters flicker to (hex)",
        type: OptionType.STRING,
        default: DEFAULT_FLICKER_COLOR
    },
    tickInterval: {
        description: "How often the flicker ticks (ms), lower = more chaotic",
        type: OptionType.SLIDER,
        default: 80,
        markers: [30, 50, 80, 120, 200, 400]
    },
    flickerChance: {
        description: "Probability a letter flickers per tick, higher = more flicker",
        type: OptionType.SLIDER,
        default: 4,
        markers: [1, 2, 4, 8, 12, 20, 40]
    },
    holdDuration: {
        description: "How long (ms) a letter stays the flicker color",
        type: OptionType.SLIDER,
        default: 120,
...
function tickFlicker() {
    ...
    const letters = document.querySelectorAll<HTMLSpanElement>('[data-flicker-letter="1"]');
    for (const letter of letters) {
        if (letter.closest("[role='textbox']") || letter.closest("[contenteditable='true']")) continue;
        if (Math.random() < chance) {
            letter.style.color = color;
            letter.style.textShadow = `0 0 4px ${color}, 0 0 10px ${color}`;
            setTimeout(() => {
                letter.style.color = "";
...
async function startBGM(fromUserGesture = false) {
    ...
    bgmAudio = new Audio(bgmSrc);  // ← use the imported base64 data URL
    bgmAudio.loop = true;
    bgmAudio.volume = (settings.store.bgmVolume ?? 35) / 100;
...
function getVisibleCandidates(): Candidate[] {
    ...
    return [...document.querySelectorAll("div[role='article']")].map(article => {
		...
		if (!c.listId || !c.videoUrl) return false;
		if (!c.username) {
			console.log("[IF] skipped: username could not be resolved for", c.listId);
			return false;
		}
		...
		const articleEl = document.querySelector(`[data-list-item-id="${c.listId}"]`);
...
async function scanForCutscene() {
    ...
    if (!candidates.length) {
        // Log every article to see why they were rejected
        [...articles].forEach(article => {
            ...
            console.log("[IF] article:", {
                ...
                mentionsMe: me ? (text.includes(`<@${me.id}>`) || text.includes(`<@!${me.id}>`)) : "no user",
    ...
    if (processedListIds.has(newest.listId)) {
        console.log("[IF] skipped: already processed listId", newest.listId);
        return;
    }

    processedListIds.add(newest.listId);
...
function startPlugin() {
    processedListIds = new Set<string>();
    lastTickInterval = settings.store.tickInterval || 80;
    ...
    if (!settings.store.bootScreenEnabled) {
        void startBGM(false);
    }
}
...
function stopPlugin() {
    ...
    document.querySelectorAll<HTMLSpanElement>("span:not([class])").forEach(span => {
...
export default definePlugin({
    ...
    authors: [{ name: "Gloomy Tree", id: 320502924830834689 }],
    settings,
